app.controller('profileCtrl', function ($scope, $ionicLoading, dataService, $cordovaCamera, $cordovaImagePicker, $http) {
    ...
    $scope.updateProfile = function (dashList) {
     //  $scope.dashList.image = 'data:image/png;base64,=dfdfgdfgdfgdfgdf'
        ...
        $http(req).then(function (res) {
           // if (res.data.message)
            ...
            if(abc.message ="User Information are Updated Succesfully")
            {
                ...
                window.localStorage.setItem("UserProfile", JSON.stringify(abc.response[0]));
                $ionicLoading.hide();
        ...
        //dataService.updateProfile(modelProfile).then(function (result) {
        //    $ionicLoading.show({
        //        noBackdrop: false,
